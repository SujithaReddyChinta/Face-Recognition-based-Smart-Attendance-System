...
for fname in os.listdir(image_dir):
    ...
    if encoding:
        ...
        # Assuming the names that appear in the attendance sheet are derived from file names of that particular person
        known_face_names.append(fname.split('.')[0])
...
def compute_persistent_homology(face_encodings):
    ...
    # Compute distance matrix in euclidean distance for face encodings
    ...
    refined_encodings = np.mean(stable_features, axis=0)  # Averaging stable features
...
if uploaded_image is not None:
    ...
    known_face_encodings = list(encodings.values())
    face_encodings = compute_persistent_homology(known_face_encodings)
    # Find face locations and encodings for detected faces
    face_locations = face_recognition.face_locations(img)
    face_encodings = face_recognition.face_encodings(img, face_locations)

    # Refine the encodings using persistent homology
